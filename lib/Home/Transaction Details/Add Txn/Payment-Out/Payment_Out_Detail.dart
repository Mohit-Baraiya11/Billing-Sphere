...
class _Payment_Out_Detail extends State<Payment_Out_Detail> {
      ...
      String newPaymentType) async
  {
    ...
    // 🔹 Adjust old party's total_amount and remove transaction
    if (oldPhoneNumber.isNotEmpty) {
      ...
      if (oldPartyEvent.snapshot.exists) {
        ...
        double newTotal = existingAmount - oldAmount; // ✅ Reverse previous amount
        ...
        print("✅ Removed transaction from old party!");
      }
    }

    // 🔹 Move transaction to new party if phone number changed
    ...
    if (newPartyEvent.snapshot.exists) {
      ...
      newTotalAmount += existingAmount; // ✅ Add new amount
    }
  ...
  void deleteTransaction() async {
    ...
    try {
      ...
      if (paymentType == "Cash") {
        ...
        await cashRef.set(cashBalance + amount);
        ...
        await bankAccRef.set(bankBalance + amount);
      ...
      if (phone.isNotEmpty) {
        ...
        if (partyEvent.snapshot.exists) {
          ...
          double newTotal = currentPartyAmount - amount;

          await partyRef.update({"total_amount": newTotal.toString()});

          // Remove transaction from party's transactions
          DatabaseReference partyTransRef =
          partyRef.child("transactions/$transactionId");
          await partyTransRef.remove();
